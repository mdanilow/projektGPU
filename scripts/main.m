...
kernel_1 = parallel.gpu.CUDAKernel('../kernels/kernel_one.ptx', '../kernels/kernel_one.cu');
kernel_2 = parallel.gpu.CUDAKernel('../kernels/kernel_two.ptx', '../kernels/kernel_two.cu');

kernel_4 = parallel.gpu.CUDAKernel('../kernels/kernel_four.ptx', '../kernels/kernel_four.cu');

im = imread(['../test_images/', test_img_name]);
if length(size(im)) > 2
    im = rgb2gray(im);
end
...
ker1_result = feval(kernel_1, im, ker1_result, im_height, im_width, NHOODTYPE);
...
final_result = label2rgb(ker4_result);
